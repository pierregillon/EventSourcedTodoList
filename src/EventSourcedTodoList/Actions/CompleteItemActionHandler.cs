...
public class CompleteItemActionHandler : ActionHandler<TodoListState.CompleteItem>
{
    ...
    public override async Task<Unit> Handle(TodoListState.CompleteItem action, CancellationToken cancellationToken)
    {
        var state = Store.GetState<TodoListState>();

        await _commandDispatcher.Dispatch(new CompleteTodoItemCommand(new TodoItemId(action.ItemId)));
